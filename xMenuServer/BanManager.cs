...
namespace xMenuServer
{
    ...
    {
        private const string BAN_KVP_PREFIX = "xmenu_ban_";
        /// <summary>
        ...
        public BanManager()
        {
            EventHandlers.Add("xMenu:TempBanPlayer", new Action<Player, int, double, string>(BanPlayer));
            EventHandlers.Add("xMenu:PermBanPlayer", new Action<Player, int, string>(BanPlayer));
            EventHandlers.Add("playerConnecting", new Action<Player, string, CallbackDelegate>(CheckForBans));
            EventHandlers.Add("xMenu:RequestPlayerUnban", new Action<Player, string>(RemoveBanRecord));
            EventHandlers.Add("xMenu:RequestBanList", new Action<Player>(SendBanList));
        }
        ...
        private void SendBanList([FromSource] Player source)
        {
            ...
            source.TriggerEvent("xMenu:SetBanList", data);
        }
        ...
        private void CheckForBans([FromSource] Player source, string playerName, CallbackDelegate kickCallback)
        {
            ...
            {
                kickCallback($"You have been permanently banned from this server. Banned by: {record.bannedBy}. Ban reason: {record.banReason}. Additional information: {xMenuShared.ConfigManager.GetSettingsString(xMenuShared.ConfigManager.Setting.xmenu_default_ban_message_information)}.");
                CancelEvent();
            }
            // Temp banned
            else
            {
                kickCallback($"You are banned from this server. Ban time remaining: {GetRemainingTimeMessage(record.bannedUntil.Subtract(DateTime.Now))}. Additional information: {xMenuShared.ConfigManager.GetSettingsString(xMenuShared.ConfigManager.Setting.xmenu_default_ban_message_information)}.");
                CancelEvent();
            }
        }
        ...
        {
            if (IsPlayerAceAllowed(source.Handle, "xMenu.OnlinePlayers.TempBan") || IsPlayerAceAllowed(source.Handle, "xMenu.Everything") || IsPlayerAceAllowed(source.Handle, "xMenu.OnlinePlayers.All"))
            {
                ...
                if (target != null)
                {
                    ...
                    if (!IsPlayerAceAllowed(target.Handle, "xMenu.DontBanMe"))
                    {
                        ...
                        target.Drop($"You are banned from this server. Ban time remaining: {timeRemaining}. Banned by: {ban.bannedBy}. Ban reason: {ban.banReason}. Aditional information: {xMenuShared.ConfigManager.GetSettingsString(xMenuShared.ConfigManager.Setting.xmenu_default_ban_message_information)}.");
                        source.TriggerEvent("xMenu:Notify", "~g~Target player successfully banned.");
                    }
                    else
                    {
                        Log("Player could not be banned because he is exempt from being banned.", LogLevel.error);
                        source.TriggerEvent("xMenu:Notify", "~r~Could not ban this player, they are exempt from being banned.");
                    }
                }
                else
                {
                    Log("Player is invalid (no longer online) and therefor the banning has failed.", LogLevel.error);
                    source.TriggerEvent("xMenu:Notify", "Could not ban this player because they already left the server.");
                }
            }
            else
            {
        ...
        private void RemoveBanRecord([FromSource] Player source, string uuid)
        {
            ...
            {
                if (IsPlayerAceAllowed(source.Handle, "xMenu.OnlinePlayers.Unban") || IsPlayerAceAllowed(source.Handle, "xMenu.OnlinePlayers.All") || IsPlayerAceAllowed(source.Handle, "xMenu.Everything"))
                {
                    ...
                    if (banRecord != null)
                    {
                        ...
                        TriggerEvent("xMenu:UnbanSuccessful", JsonConvert.SerializeObject(banRecord).ToString());
                    }
                }
                else
                {
                    BanCheater(source);
                    Debug.WriteLine($"^3[xMenu] [WARNING] [BAN] ^7Player {JsonConvert.SerializeObject(source)} did not have the required permissions, but somehow triggered the unban event. Missing permissions: xMenu.OnlinePlayers.Unban (is ace allowed: {IsPlayerAceAllowed(source.Handle, "xMenu.OnlinePlayers.Unban")})\n");
                }
            }
            else
            {
                Debug.WriteLine("^3[xMenu] [WARNING] ^7The unban event was triggered, but no valid source was provided. Nobody has been unbanned.");
            }
        }
        ...
        {
            bool enabled = xMenuShared.ConfigManager.GetSettingsBool(xMenuShared.ConfigManager.Setting.xmenu_auto_ban_cheaters);
            if (enabled)
            {
                string reason = xMenuShared.ConfigManager.GetSettingsString(xMenuShared.ConfigManager.Setting.xmenu_auto_ban_cheaters_ban_message);

                if (string.IsNullOrEmpty(reason))
                {
                    reason = $"You have been automatically banned. If you believe this was done by error, please contact the server owner for support. Aditional information: {xMenuShared.ConfigManager.GetSettingsString(xMenuShared.ConfigManager.Setting.xmenu_default_ban_message_information)}.";
                }
                ...
                TriggerEvent("xMenu:BanCheaterSuccessful", JsonConvert.SerializeObject(ban).ToString());
                BanLog($"A cheater has been banned. {JsonConvert.SerializeObject(ban)}");

                source.TriggerEvent("xMenu:GoodBye"); // this is much more fun than just kicking them.
        ...
        {
            if (xMenuShared.ConfigManager.GetSettingsBool(xMenuShared.ConfigManager.Setting.xmenu_log_ban_actions))
            {
                ...
                Debug.WriteLine("^2[xMenu] [SUCCESS] [BAN]^7 " + banActionMessage);
            }
        }
