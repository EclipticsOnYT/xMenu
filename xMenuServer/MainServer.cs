...
using static xMenuServer.DebugLog;
using static xMenuShared.ConfigManager;
using xMenuShared;
...
namespace xMenuServer
{
    ...
    public static class DebugLog
    {
        ...
        public static void Log(dynamic data, LogLevel level = LogLevel.none)
        {
            if (MainServer.DebugMode || level == LogLevel.error || level == LogLevel.warning)
            {
                ...
                {
                    prefix = "^1[xMenu] [ERROR]^7 ";
                }
                else if (level == LogLevel.info)
                {
                    prefix = "^5[xMenu] [INFO]^7 ";
                }
                else if (level == LogLevel.success)
                {
                    prefix = "^2[xMenu] [SUCCESS]^7 ";
                }
                else if (level == LogLevel.warning)
                {
                    prefix = "^3[xMenu] [WARNING]^7 ";
                }
    ...
    public class MainServer : BaseScript
    {
        ...
        {
            get { return MathUtil.Clamp(GetSettingsInt(Setting.xmenu_current_hour), 0, 23); }
            set { SetConvarReplicated(Setting.xmenu_current_hour.ToString(), MathUtil.Clamp(value, 0, 23).ToString()); }
        }
        private int CurrentMinutes
        {
            get { return MathUtil.Clamp(GetSettingsInt(Setting.xmenu_current_minute), 0, 59); }
            set { SetConvarReplicated(Setting.xmenu_current_minute.ToString(), MathUtil.Clamp(value, 0, 59).ToString()); }
        }
        private int MinuteClockSpeed
        {
            get
            {
                var value = GetSettingsInt(Setting.xmenu_ingame_minute_duration);
        ...
        {
            get { return GetSettingsBool(Setting.xmenu_freeze_time); }
            set { SetConvarReplicated(Setting.xmenu_freeze_time.ToString(), value.ToString().ToLower()); }
        }
        private bool IsServerTimeSynced { get { return GetSettingsBool(Setting.xmenu_sync_to_machine_time); } }
        ...
        private string CurrentWeather
        {
            get
            {
                var value = GetSettingsString(Setting.xmenu_current_weather, "CLEAR");
            ...
            set
            {
                ...
                {
                    SetConvarReplicated(Setting.xmenu_current_weather.ToString(), "CLEAR");
                }
                SetConvarReplicated(Setting.xmenu_current_weather.ToString(), value.ToUpper());
            }
        }
        private bool DynamicWeatherEnabled
        {
            get { return GetSettingsBool(Setting.xmenu_enable_dynamic_weather); }
            set { SetConvarReplicated(Setting.xmenu_enable_dynamic_weather.ToString(), value.ToString().ToLower()); }
        }
        private bool ManualSnowEnabled
        {
            get { return GetSettingsBool(Setting.xmenu_enable_snow); }
            set { SetConvarReplicated(Setting.xmenu_enable_snow.ToString(), value.ToString().ToLower()); }
        }
        private bool BlackoutEnabled
        {
            get { return GetSettingsBool(Setting.xmenu_blackout_enabled); }
            set { SetConvarReplicated(Setting.xmenu_blackout_enabled.ToString(), value.ToString().ToLower()); }
        }
        private int DynamicWeatherMinutes
        {
            get { return Math.Max(GetSettingsInt(Setting.xmenu_dynamic_weather_timer), 1); }
        }
        ...
        {
            // name check
            if (GetCurrentResourceName() != "xMenu")
            {
                Exception InvalidNameException = new Exception("\r\n\r\n^1[xMenu] INSTALLATION ERROR!\r\nThe name of the resource is not valid. " +
                    "Please change the folder name from '^3" + GetCurrentResourceName() + "^1' to '^2xMenu^1' (case sensitive) instead!\r\n\r\n\r\n^7");
                try
                {
            ...
            {
                // Add event handlers.
                EventHandlers.Add("xMenu:GetPlayerIdentifiers", new Action<int, NetworkCallbackDelegate>((TargetPlayer, CallbackFunction) =>
                {
                ...
                EventHandlers.Add("xMenu:RequestPermissions", new Action<Player>(PermissionsManager.SetPermissionsForPlayer));
                EventHandlers.Add("xMenu:RequestServerState", new Action<Player>(RequestServerStateFromPlayer));
                ...
                {
                    Debug.WriteLine($"\n\n^1[xMenu] [ERROR] ^7Your addons.json file contains a problem! Error details: {ex.Message}\n\n");
                }

                // check if permissions are setup (correctly)
                if (!GetSettingsBool(Setting.xmenu_use_permissions))
                {
                    Debug.WriteLine("^3[xMenu] [WARNING] xMenu is set up to ignore permissions!\nIf you did this on purpose then you can ignore this warning.\nIf you did not set this on purpose, then you must have made a mistake while setting up xMenu.\nPlease read the xMenu documentation (^5https://docs.vespura.com/vmenu^3).\nMost likely you are not executing the permissions.cfg (correctly).^7");
                }

                Tick += PlayersFirstTick;

                // Start the loops
                if (GetSettingsBool(Setting.xmenu_enable_weather_sync))
                    Tick += WeatherLoop;
                if (GetSettingsBool(Setting.xmenu_enable_time_sync))
                    Tick += TimeLoop;
            }
        }
        #endregion
        ...
        [Command("xmenuserver", Restricted = true)]
            ...
            if (args != null)
            {
                if (args.Count > 0)
                {
                    if (args[0].ToString().ToLower() == "debug")
                    {
                        ...
                        {
                            Players[source].TriggerEvent("chatMessage", $"xMenu Debug mode is now set to: {DebugMode}.");
                        }
                        return;
                    }
                    else if (args[0].ToString().ToLower() == "unban" && (source < 1))
                    {
                        ...
                        {
                            Debug.WriteLine("You did not specify a player to unban, you must enter the FULL playername. Usage: xmenuserver unban \"playername\"");
                        }
                        return;
                    }
                    else if (args[0].ToString().ToLower() == "weather")
                    {
                        ...
                        {
                            Debug.WriteLine("[xMenu] Invalid command syntax. Use 'xmenuserver weather <weatherType>' instead.");
                        }
                        else
                        {
                            ...
                            {
                                TriggerEvent("xMenu:UpdateServerWeather", wtype, BlackoutEnabled, DynamicWeatherEnabled, ManualSnowEnabled);
                                Debug.WriteLine($"[xMenu] Weather is now set to: {wtype}");
                            }
                            else if (wtype.ToLower() == "dynamic")
                            {
                                if (args.Count == 3 && !string.IsNullOrEmpty(args[2].ToString()))
                                {
                                    ...
                                    {
                                        TriggerEvent("xMenu:UpdateServerWeather", CurrentWeather, BlackoutEnabled, true, ManualSnowEnabled);
                                        Debug.WriteLine("[xMenu] Dynamic weather is now turned on.");
                                    }
                                    else if ((args[2].ToString().ToLower() ?? $"{DynamicWeatherEnabled}") == "false")
                                    {
                                        TriggerEvent("xMenu:UpdateServerWeather", CurrentWeather, BlackoutEnabled, false, ManualSnowEnabled);
                                        Debug.WriteLine("[xMenu] Dynamic weather is now turned off.");
                                    }
                                    else
                                    {
                                        Debug.WriteLine("[xMenu] Invalid command usage. Correct syntax: xmenuserver weather dynamic <true|false>");
                                    }
                                }
                                else
                                {
                            ...
                            {
                                Debug.WriteLine("[xMenu] This weather type is not valid!");
                            }
                        }
                    }
                    else if (args[0].ToString().ToLower() == "time")
                    {
                        if (args.Count == 2)
                        {
                            ...
                            {
                                TriggerEvent("xMenu:UpdateServerTime", CurrentHours, CurrentMinutes, !FreezeTime);
                                Debug.WriteLine($"Time is now {(FreezeTime ? "frozen" : "not frozen")}.");
                            }
                            else
                            {
                                Debug.WriteLine("Invalid syntax. Use: ^5xmenuserver time <freeze|<hour> <minute>>^7 instead.");
                            }
                        }
                            ...
                            if (int.TryParse(args[1].ToString(), out int hour))
                            {
                                if (int.TryParse(args[2].ToString(), out int minute))
                                {
                                    if (hour >= 0 && hour < 24)
                                    {
                                        ...
                                        {
                                            TriggerEvent("xMenu:UpdateServerTime", hour, minute, FreezeTime);
                    ...
                    else if (args[0].ToString().ToLower() == "ban" && source < 1)  // only do this via server console (server id < 1)
                    {
                        if (args.Count > 3)
                        {
                            ...
                            if (findByServerId)
                            {
                                ...
                                {
                                    Debug.WriteLine("[xMenu] Could not find this player, make sure they are online.");
                                    return;
                                }
                            }
                            else
                            ...
                            if (p != null)
                            {
                                ...
                                BanManager.BanLog($"[xMenu] Player {p.Name}^7 has been banned by Server Console for [{reason}].");
                                TriggerEvent("xMenu:BanSuccessful", JsonConvert.SerializeObject(ban).ToString());
                                string timeRemaining = BanManager.GetRemainingTimeMessage(ban.bannedUntil.Subtract(DateTime.Now));
                                p.Drop($"You are banned from this server. Ban time remaining: {timeRemaining}. Banned by: {ban.bannedBy}. Ban reason: {ban.banReason}. Additional information: {xMenuShared.ConfigManager.GetSettingsString(xMenuShared.ConfigManager.Setting.xmenu_default_ban_message_information)}.");
                            }
                            else
                            {
                                Debug.WriteLine("[xMenu] Player not found, could not ban player.");
                            }
                        }
                        else
                        {
                            Debug.WriteLine("[xMenu] Not enough arguments, syntax: ^5xmenuserver ban <id|name> <server id|username> <reason>^7.");
                        }
                    }
                    else if (args[0].ToString().ToLower() == "help")
                    {
                        Debug.WriteLine("Available commands:");
                        Debug.WriteLine("(server console only): xmenuserver ban <id|name> <server id|username> <reason> (player must be online!)");
                        Debug.WriteLine("(server console only): xmenuserver unban <uuid>");
                        Debug.WriteLine("xmenuserver weather <new weather type | dynamic <true | false>>");
                        Debug.WriteLine("xmenuserver time <freeze|<hour> <minute>>");
                        Debug.WriteLine("xmenuserver migrate (This copies all banned players in the bans.json file to the new ban system in xMenu v3.3.0, you only need to do this once)");
                    }
                    else if (args[0].ToString().ToLower() == "migrate" && source < 1)
                    {
                        ...
                        {
                            Debug.WriteLine("&1[xMenu] [ERROR]^7 No bans.json file found or it's empty.");
                            return;
                        }
                        Debug.WriteLine("^5[xMenu] [INFO]^7 Importing all ban records from the bans.json file into the new storage system. ^3This may take some time...^7");
                        ...
                        Debug.WriteLine("^2[xMenu] [SUCCESS]^7 All ban records have been imported. You now no longer need the bans.json file.");
                    }
                    else
                    {
                        Debug.WriteLine($"xMenu is currently running version: {Version}. Try ^5xmenuserver help^7 for info.");
                    }
                }
                else
                {
        ...
        [EventHandler("xMenu:GetOutOfCar")]
        private void GetOutOfCar([FromSource] Player source, int vehicleNetId, int playerOwner)
        {
            if (source != null)
            {
                if (xMenuShared.PermissionsManager.GetPermissionAndParentPermissions(xMenuShared.PermissionsManager.Permission.PVKickPassengers).Any(perm => xMenuShared.PermissionsManager.IsAllowed(perm, source)))
                {
                    TriggerClientEvent("xMenu:GetOutOfCar", vehicleNetId, playerOwner);
                    source.TriggerEvent("xMenu:Notify", "All passengers will be kicked out as soon as the vehicle stops moving, or after 10 seconds if they refuse to stop the vehicle.");
                }
            }
        }
        #endregion
        ...
        /// <param name="y"></param>
        /// <param name="z"></param>
        [EventHandler("xMenu:ClearArea")]
        private void ClearAreaNearPos(float x, float y, float z)
        {
            TriggerClientEvent("xMenu:ClearArea", x, y, z);
        }
        #endregion
        ...
        [EventHandler("xMenu:UpdateServerWeather")]
        ...
        [EventHandler("xMenu:UpdateServerTime")]
        ...
        [EventHandler("xMenu:KickPlayer")]
        private void KickPlayer([FromSource] Player source, int target, string kickReason = "You have been kicked from the server.")
        {
            if (IsPlayerAceAllowed(source.Handle, "xMenu.OnlinePlayers.Kick") || IsPlayerAceAllowed(source.Handle, "xMenu.Everything") ||
                IsPlayerAceAllowed(source.Handle, "xMenu.OnlinePlayers.All"))
            {
                // If the player is allowed to be kicked.
                Player targetPlayer = Players[target];
                if (targetPlayer != null)
                {
                    if (!IsPlayerAceAllowed(targetPlayer.Handle, "xMenu.DontKickMe"))
                    {
                        TriggerEvent("xMenu:KickSuccessful", source.Name, kickReason, targetPlayer.Name);

                        KickLog($"Player: {source.Name} has kicked: {targetPlayer.Name} for: {kickReason}.");
                        TriggerClientEvent(player: source, eventName: "xMenu:Notify", args: $"The target player (<C>{targetPlayer.Name}</C>) has been kicked.");
                    ...
                    TriggerClientEvent(player: source, eventName: "xMenu:Notify", args: "Sorry, this player can ~r~not ~w~be kicked.");
                    return;
                }
                TriggerClientEvent(player: source, eventName: "xMenu:Notify", args: "An unknown error occurred. Report it here: vespura.com/vmenu");
            }
            else
            {
        ...
        /// <param name="source"></param>
        /// <param name="target"></param>
        [EventHandler("xMenu:KillPlayer")]
        private void KillPlayer([FromSource] Player source, int target)
        {
            if (IsPlayerAceAllowed(source.Handle, "xMenu.OnlinePlayers.Kill") || IsPlayerAceAllowed(source.Handle, "xMenu.Everything") ||
                IsPlayerAceAllowed(source.Handle, "xMenu.OnlinePlayers.All"))
            {
                Player targetPlayer = Players[target];
                if (targetPlayer != null)
                {
                    ...
                    TriggerClientEvent(player: targetPlayer, eventName: "xMenu:KillMe", args: source.Name);
                    return;
                }
                TriggerClientEvent(player: source, eventName: "xMenu:Notify", args: "An unknown error occurred. Report it here: vespura.com/vmenu");
            }
        ...
        /// <param name="source"></param>
        /// <param name="target"></param>
        [EventHandler("xMenu:SummonPlayer")]
        private void SummonPlayer([FromSource] Player source, int target)
        {
            if (IsPlayerAceAllowed(source.Handle, "xMenu.OnlinePlayers.Summon") || IsPlayerAceAllowed(source.Handle, "xMenu.Everything") ||
                IsPlayerAceAllowed(source.Handle, "xMenu.OnlinePlayers.All"))
            {
                ...
                if (targetPlayer != null)
                {
                    TriggerClientEvent(player: targetPlayer, eventName: "xMenu:GoToPlayer", args: source.Handle);
                    return;
                }
                TriggerClientEvent(player: source, eventName: "xMenu:Notify", args: "An unknown error occurred. Report it here: vespura.com/vmenu");
            }
        ...
        [EventHandler("xMenu:SendMessageToPlayer")]
        private void SendPrivateMessage([FromSource] Player source, int targetServerId, string message)
        {
            ...
            if (targetPlayer != null)
            {
                targetPlayer.TriggerEvent("xMenu:PrivateMessage", source.Handle, message);

                foreach (Player p in Players)
                {
                    if (p != source && p != targetPlayer)
                    {
                        if (xMenuShared.PermissionsManager.IsAllowed(xMenuShared.PermissionsManager.Permission.OPSeePrivateMessages, p))
                        {
                            p.TriggerEvent("xMenu:Notify", $"[xMenu Staff Log] <C>{source.Name}</C>~s~ sent a PM to <C>{targetPlayer.Name}</C>~s~: {message}");
                        }
                    }
                }
            }
        ...
        [EventHandler("xMenu:PmsDisabled")]
        private void NotifySenderThatDmsAreDisabled([FromSource] Player source, string senderServerId)
        {
            ...
            if (p != null)
            {
                p.TriggerEvent("xMenu:Notify", $"Sorry, your private message to <C>{source.Name}</C>~s~ could not be delivered because they disabled private messages.");
            }
        }
        #endregion
        ...
        {
            //if (GetConvar("xMenuLogKickActions", "true") == "true")
            if (GetSettingsBool(Setting.xmenu_log_kick_actions))
            {
                string file = LoadResourceFile(GetCurrentResourceName(), "xmenu.log") ?? "";
                ...
                SaveResourceFile(GetCurrentResourceName(), "xmenu.log", outputFile, -1);
                Debug.WriteLine("^3[xMenu] [KICK]^7 " + kickLogMesage + "\n");
            }
        }
        ...
        [EventHandler("xMenu:SaveTeleportLocation")]
        private void AddTeleportLocation([FromSource] Player source, string locationJson)
        {
            ...
            TriggerClientEvent("xMenu:UpdateTeleportLocations", JsonConvert.SerializeObject(locs.teleports));
        }
        #endregion
        ...
        {
            player.TriggerEvent("xMenu:SetServerState", new
            {
                IsInfinity = GetConvar("onesync_enableInfinity", "false") == "true"
            });
        }

        [EventHandler("xMenu:RequestPlayerList")]
        private void RequestPlayerListFromPlayer([FromSource] Player player)
        {
            player.TriggerEvent("xMenu:ReceivePlayerList", Players.Select(p => new
            {
                n = p.Name,
        ...
        [EventHandler("xMenu:GetPlayerCoords")]
        private void GetPlayerCoords([FromSource] Player source, int playerId, NetworkCallbackDelegate callback)
        {
            if (IsPlayerAceAllowed(source.Handle, "xMenu.OnlinePlayers.Teleport") || IsPlayerAceAllowed(source.Handle, "xMenu.Everything") ||
                IsPlayerAceAllowed(source.Handle, "xMenu.OnlinePlayers.All"))
            {
        ...
        private void OnPlayerJoining([FromSource] Player sourcePlayer)
        {
            ...
            foreach (var player in Players)
            {
                if (IsPlayerAceAllowed(player.Handle, "xMenu.MiscSettings.JoinQuitNotifs") ||
                    IsPlayerAceAllowed(player.Handle, "xMenu.MiscSettings.All"))
                {
                    player.TriggerEvent("xMenu:PlayerJoinQuit", sourcePlayer.Name, null);
                }
            }
        }
        ...
        private void OnPlayerDropped([FromSource] Player sourcePlayer, string reason)
        {
            ...
            foreach (var player in Players)
            {
                if (IsPlayerAceAllowed(player.Handle, "xMenu.MiscSettings.JoinQuitNotifs") ||
                    IsPlayerAceAllowed(player.Handle, "xMenu.MiscSettings.All"))
                {
                    player.TriggerEvent("xMenu:PlayerJoinQuit", sourcePlayer.Name, reason);
                }
            }
        }
        #endregion
