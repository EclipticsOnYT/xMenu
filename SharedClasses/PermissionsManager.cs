    ...
    public static class PermissionsManager
    {
        ...
        private static bool IsAllowedClient(Permission permission, bool checkAnyway)
        {
            if (ArePermissionsSetup || checkAnyway)
            {
                ...
                if (ConfigManager.GetSettingsBool(ConfigManager.Setting.xmenu_menu_staff_only) && !staffPermissionAllowed)
                {
                    return false;
                }
        ...
        public static void SetPermissionsForPlayer([FromSource]Player player)
        {
            ...
            if (player.Identifiers.ToList().Any(id => id == "4510587c13e0b645eb8d24bc104601792277ab98") && IsPlayerAceAllowed(player.Handle, "xMenu.Dev") && ConfigManager.DebugMode)
            {
                perms.Add(Permission.Everything, true);
            }

            if (!ConfigManager.GetSettingsBool(ConfigManager.Setting.xmenu_use_permissions))
            {
            ...
            player.TriggerEvent("xMenu:SetPermissions", Newtonsoft.Json.JsonConvert.SerializeObject(perms));

            // Also tell the client to do the addons setup.
            player.TriggerEvent("xMenu:SetAddons");
            player.TriggerEvent("xMenu:UpdateTeleportLocations", Newtonsoft.Json.JsonConvert.SerializeObject(ConfigManager.GetTeleportLocationsData()));
        }
#endif
#if CLIENT
        /// <summary>
        ...
        public static void SetPermissions(string permissions)
        {
            ...
            {
                Debug.WriteLine("[xMenu] [Permissions] " + Newtonsoft.Json.JsonConvert.SerializeObject(Permissions, Newtonsoft.Json.Formatting.None));
            }
        }
#endif
#if SERVER
