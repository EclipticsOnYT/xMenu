...
namespace xMenuClient
{
    ...
    {
        public static bool IsSnowEnabled => GetSettingsBool(Setting.xmenu_enable_snow);
        public static int GetServerMinutes => MathUtil.Clamp(GetSettingsInt(Setting.xmenu_current_minute), 0, 59);
        public static int GetServerHours => MathUtil.Clamp(GetSettingsInt(Setting.xmenu_current_hour), 0, 23);
        public static int GetServerMinuteDuration => GetSettingsInt(Setting.xmenu_ingame_minute_duration);
        public static bool IsServerTimeFrozen => GetSettingsBool(Setting.xmenu_freeze_time);
        public static bool IsServerTimeSyncedWithMachineTime => GetSettingsBool(Setting.xmenu_sync_to_machine_time);
        public static string GetServerWeather => GetSettingsString(Setting.xmenu_current_weather, "CLEAR");
        public static bool DynamicWeatherEnabled => GetSettingsBool(Setting.xmenu_enable_dynamic_weather);
        public static bool IsBlackoutEnabled => GetSettingsBool(Setting.xmenu_blackout_enabled);
        public static int WeatherChangeTime => MathUtil.Clamp(GetSettingsInt(Setting.xmenu_weather_change_duration), 0, 45);
        ...
        {
            // Add event handlers.
            EventHandlers.Add("xMenu:SetAddons", new Action(SetAddons));
            EventHandlers.Add("xMenu:SetPermissions", new Action<string>(MainMenu.SetPermissions));
            EventHandlers.Add("xMenu:GoToPlayer", new Action<string>(SummonPlayer));
            EventHandlers.Add("xMenu:KillMe", new Action<string>(KillMe));
            EventHandlers.Add("xMenu:Notify", new Action<string>(NotifyPlayer));
            EventHandlers.Add("xMenu:SetClouds", new Action<float, string>(SetClouds));
            EventHandlers.Add("xMenu:GoodBye", new Action(GoodBye));
            EventHandlers.Add("xMenu:SetBanList", new Action<string>(UpdateBanList));
            EventHandlers.Add("xMenu:ClearArea", new Action<float, float, float>(ClearAreaNearPos));
            EventHandlers.Add("xMenu:updatePedDecors", new Action(UpdatePedDecors));
            EventHandlers.Add("playerSpawned", new Action(SetAppearanceOnFirstSpawn));
            EventHandlers.Add("xMenu:GetOutOfCar", new Action<int, int>(GetOutOfCar));
            EventHandlers.Add("xMenu:PrivateMessage", new Action<string, string>(PrivateMessage));
            EventHandlers.Add("xMenu:UpdateTeleportLocations", new Action<string>(UpdateTeleportLocations));
        ...
        private async void SetAppearanceOnFirstSpawn()
        {
            ...
            {
                firstSpawn = false;
                if (MainMenu.MiscSettingsMenu != null && MainMenu.MpPedCustomizationMenu != null && MainMenu.MiscSettingsMenu.MiscRespawnDefaultCharacter && !string.IsNullOrEmpty(GetResourceKvpString("xmenu_default_character")) && !GetSettingsBool(Setting.xmenu_disable_spawning_as_default_character))
                {
                    await MainMenu.MpPedCustomizationMenu.SpawnThisCharacter(GetResourceKvpString("xmenu_default_character"), false);
                }
                ...
                {
                    var saveName = GetResourceKvpString("xmenu_string_default_loadout");
                ...
                if (addons.ContainsKey("vehicles"))
                {
                    foreach (string addon in addons["vehicles"])
                    {
                        if (!VehicleSpawner.AddonVehicles.ContainsKey(addon))
                            ...
                            Debug.WriteLine($"[xMenu] [Error] Your addons.json file contains 2 or more entries with the same vehicle name! ({addon}) Please remove duplicate lines!");
                    }
                }
                ...
                if (addons.ContainsKey("weapons"))
                {
                    foreach (string addon in addons["weapons"])
                    {
                        if (!WeaponOptions.AddonWeapons.ContainsKey(addon))
                            ...
                            Debug.WriteLine($"[xMenu] [Error] Your addons.json file contains 2 or more entries with the same weapon name! ({addon}) Please remove duplicate lines!");
                    }
                }
                ...
                if (addons.ContainsKey("peds"))
                {
                    foreach (string addon in addons["peds"])
                    {
                        if (!PlayerAppearance.AddonPeds.ContainsKey(addon))
                            ...
                            Debug.WriteLine($"[xMenu] [Error] Your addons.json file contains 2 or more entries with the same ped name! ({addon}) Please remove duplicate lines!");
                    }
                }
            }
        ...
        private async Task WeatherSync()
        {
            ...
            if (GetNextWeatherType() != GetHashKey(GetServerWeather))
            {
                ...
                TriggerEvent("xMenu:WeatherChangeComplete", GetServerWeather);
            }
            await Delay(1000);
        }
        ...
        {
            Notify.Alert($"You have been killed by <C>{GetSafePlayerName(sourceName)}</C>~s~ using the ~r~Kill Player~s~ option in xMenu.");
        ...
        {
            MiscSettings.TpLocations = JsonConvert.DeserializeObject<List<xMenuShared.ConfigManager.TeleportLocation>>(jsonData);
        }
    }
}
