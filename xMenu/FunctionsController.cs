...
using static xMenuClient.data.PedModels;
...
namespace xMenuClient
{
    ...
    class FunctionsController : BaseScript
    {
        ...
        /// </summary>
        [EventHandler("xMenu:SetupTickFunctions")]
        public void SetupTickFunctions()
        {
            ...
            // Configuration based
            if (!GetSettingsBool(Setting.xmenu_disable_spawning_as_default_character))
            {
                Tick += RestorePlayerAfterBeingDead;
            }
            if (!GetSettingsBool(Setting.xmenu_disable_entity_outlines_tool))
            {
                Tick += SlowMiscTick;
            ...
            if (GetSettingsBool(Setting.xmenu_enable_animals_spawn_menu))
            {
        ...
        private async Task GcTick()
        {
            if (GetGameTimer() - gcTimer > 60000)
            {
                ...
                GC.Collect();
                Log($"[xMenu] GC at {GetGameTimer()} ({GetTimeAsString(GetGameTimer())}).");
        ...
        private async Task PlayerOptions()
        {
            ...
            if (GetSettingsBool(Setting.xmenu_handle_invisibility) && MainMenu.PlayerOptionsMenu.PlayerInvisible && IsAllowed(Permission.POInvisible))
            {
            ...
            if (IsPedInAnyVehicle(Game.PlayerPed.Handle, true))
            {
                ...
                if (veh != null && veh.Exists())
                {
                    ...
                    if (SwitchedVehicle)
                    {
                        ...
                        if (!GetSettingsBool(Setting.xmenu_use_els_compatibility_mode))
                        {
                            // No Siren Toggle
                    ...
                    if (MainMenu.VehicleOptionsMenu.VehicleInfiniteFuel && DecorIsRegisteredAsType("_Fuel_Level", 1) && IsAllowed(Permission.VOInfiniteFuel))
                    {
                        ...
                        if (maxFuelLevel > 5f && currentFuelLevel < (maxFuelLevel * 0.95f))
                        {
                            ...
                            {
                                Debug.WriteLine(@"[CRITICAL] A critical bug in one of your scripts was detected. xMenu is unable to set or register a decorator's value because another resource has already registered 1.5k or more decorators. xMenu will NOT work as long as this bug in your other scripts is unsolved. Please fix your other scripts. This is *NOT* caused by or fixable by xMenu!!!");
        ...
        /// </summary>
        /// <returns></returns>
        [EventHandler("xMenu:PlayerJoinQuit")]
        ...
        private async Task RestorePlayerAfterBeingDead()
        {
            if (MainMenu.MiscSettingsMenu != null && Game.PlayerPed.IsDead)
            {
                ...
                {
                    if (!string.IsNullOrEmpty(GetResourceKvpString("xmenu_default_character")))
                    {
                        restoreDefault = true;
                    }
                    else
                ...
                if (!restoreDefault)
                {
                    ...
                    {
                        await SavePed("xMenu_tmp_saved_ped");
                    }
                }
                ...
                if (MainMenu.MiscSettingsMenu.RestorePlayerWeapons && IsAllowed(Permission.MSRestoreWeapons) || (MainMenu.WeaponLoadoutsMenu != null && MainMenu.WeaponLoadoutsMenu.WeaponLoadoutsSetLoadoutOnRespawn && IsAllowed(Permission.WLEquipOnRespawn)))
                {
                    ...
                    if (SaveWeaponLoadout("xmenu_temp_weapons_loadout_before_respawn"))
                    {
                        Log($"weapons saved {GetResourceKvpString("xmenu_temp_weapons_loadout_before_respawn")}");
                    }
                    else
                    {
                ...
                else
                {
                    ...
                    {
                        LoadSavedPed("xMenu_tmp_saved_ped", false);
                    }
                }
                ...
                {
                    await SpawnWeaponLoadoutAsync("xmenu_temp_weapons_loadout_before_respawn", true, false, false);
                    Log("weapons restored, deleting kvp");
                    DeleteResourceKvp("xmenu_temp_weapons_loadout_before_respawn");
                }
            }
        }
        #endregion
        ...
        {
            if (DecorIsRegisteredAsType("xmenu_player_blip_sprite_id", 3))
            {
                int sprite = 1;
                ...
                try
                {
                    DecorSetInt(Game.PlayerPed.Handle, "xmenu_player_blip_sprite_id", sprite);
                }
                catch (Exception e)
                {
                    Debug.WriteLine(@"[CRITICAL] A critical bug in one of your scripts was detected. xMenu is unable to set or register a decorator's value because another resource has already registered 1.5k or more decorators. xMenu will NOT work as long as this bug in your other scripts is unsolved. Please fix your other scripts. This is *NOT* caused by or fixable by xMenu!!!");
                            ...
                            if (enabled)
                            {
                                if (!p.IsLocal)
                                {
                                    ...
                                    if (p.Character.Position.DistanceToSquared2D(Game.PlayerPed.Position) < 500000 || Game.IsPaused)
                                    {
                                        ...
                                        if (DecorExistOn(p.Character.Handle, "xmenu_player_blip_sprite_id"))
                                        {
                                            int decorSprite = DecorGetInt(p.Character.Handle, "xmenu_player_blip_sprite_id");
            ...
            else // decorator does not exist.
            {
                try
                {
                    DecorRegister("xmenu_player_blip_sprite_id", 3);
                }
                catch (Exception e)
                {
                    Debug.WriteLine(@"[CRITICAL] A critical bug in one of your scripts was detected. xMenu is unable to set or register a decorator's value because another resource has already registered 1.5k or more decorators. xMenu will NOT work as long as this bug in your other scripts is unsolved. Please fix your other scripts. This is *NOT* caused by or fixable by xMenu!!!");
                ...
                while (!DecorIsRegisteredAsType("xmenu_player_blip_sprite_id", 3))
                {
                    await Delay(0);
                }
            }
        ...
        private float playerNamesDistance = GetSettingsFloat(Setting.xmenu_player_names_distance) > 10f ? GetSettingsFloat(Setting.xmenu_player_names_distance) : 500f;
            ...
            if (MainMenu.PermissionsSetupComplete)
            {
                ...
                if (N_0xd40aac51e8e4c663((uint)compHash) > 0) // helmet has visor.
                {
                    ...
                    while (GetEntityAnimCurrentTime(Game.PlayerPed.Handle, animDict, animName) <= 0.0f)
                    {
                        ...
                        {
                            ClearPedTasks(Game.PlayerPed.Handle);
                            Debug.WriteLine("[xMenu] [WARNING] Waiting for animation to start took too long. Preventing hanging of function. Dbg: fault in location 1.");
                            return;
                        }
                        await Delay(0);
                    }
                    ...
                    while (GetEntityAnimCurrentTime(Game.PlayerPed.Handle, animDict, animName) > 0.0f)
                    {
                        ...
                        {
                            ClearPedTasks(Game.PlayerPed.Handle);
                            Debug.WriteLine("[xMenu] [WARNING] Waiting for animation duration took too long. Preventing hanging of function. Dbg: fault in location 2.");
                            return;
                        }
