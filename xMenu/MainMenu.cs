    ...
    public class MainMenu : BaseScript
    {
        ...
        public MainMenu()
        {
            ...
            while (true)
            {
                ...
                if (k == "xmenu_cleanup_version")
                {
                    if (GetResourceKvpInt("xmenu_cleanup_version") >= currentCleanupVersion)
                    {
            ...
            {
                SetResourceKvpInt("xmenu_cleanup_version", currentCleanupVersion);
                foreach (string kvp in tmp_kvp_names)
                {
                    if (currentCleanupVersion == 1 || currentCleanupVersion == 2)
                    {
                        if (!kvp.StartsWith("settings_") && !kvp.StartsWith("xmenu") && !kvp.StartsWith("veh_") && !kvp.StartsWith("ped_") && !kvp.StartsWith("mp_ped_"))
                        {
                            DeleteResourceKvp(kvp);
                            Debug.WriteLine($"[xMenu] [cleanup id: 1] Removed unused (old) KVP: {kvp}.");
                        }
                    }
                    if (currentCleanupVersion == 2)
                    {
                        ...
                        {
                            DeleteResourceKvp(kvp);
                            Debug.WriteLine($"[xMenu] [cleanup id: 2] Removed unused (old) KVP: {kvp}.");
                        }
                    }
                }
                Debug.WriteLine("[xMenu] Cleanup of old unused KVP items completed.");
            }
            #endregion
            ...
            RegisterCommand("xmenuclient", new Action<dynamic, List<dynamic>, string>((dynamic source, List<dynamic> args, string rawCommand) =>
            {
                if (args != null)
                {
                    if (args.Count > 0)
                    {
                        if (args[0].ToString().ToLower() == "debug")
                        {
                            ...
                            if (DebugMode)
                            {
                                SetRichPresence($"Debugging xMenu {Version}!");
                            }
                            else
                            {
                        ...
                        else if (args[0].ToString().ToLower() == "dump")
                        {
                            ...
                            Debug.WriteLine("\n\n\n########################### xMenu ###########################");
                            Debug.WriteLine($"Running xMenu Version: {Version}, Experimental features: {EnableExperimentalFeatures}, Debug mode: {DebugMode}.");
                            ...
                            while (true)
                            {
                                ...
                                //if (!k.StartsWith("settings_") && !k.StartsWith("xmenu") && !k.StartsWith("veh_") && !k.StartsWith("ped_") && !k.StartsWith("mp_ped_"))
                                //{
                                //    DeleteResourceKvp(k);
                                //}
                                names.Add(k);
                            ...
                            foreach (var kvp in names)
                            {
                                ...
                                else if (kvp == "xmenu_cleanup_version") // int
                                {
                                    type = 2;
                                }
                                switch (type)
                            ...
                            Debug.WriteLine("\n\nDumping xmenu server configuration settings:");
                    ...
                    {
                        Notify.Custom($"xMenu is currently running version: {Version}.");
                    }
                }
            }), false);
            ...
            if (GetCurrentResourceName() != "xMenu")
            {
                ...
                throw new Exception("\n[xMenu] INSTALLATION ERROR!\nThe name of the resource is not valid. Please change the folder name from '" + GetCurrentResourceName() + "' to 'xMenu' (case sensitive)!\n");
            }
            else
            {
                Tick += OnTick;
            ...
            TriggerServerEvent("xMenu:RequestPermissions");

            // Request server state from the server.
            TriggerServerEvent("xMenu:RequestServerState");
        }

        #region Infinity bits
        [EventHandler("xMenu:SetServerState")]
        ...
        [EventHandler("xMenu:ReceivePlayerList")]
        ...
        public static async Task<Vector3> RequestPlayerCoordinates(int serverId)
        {
            ...
            TriggerServerEvent("xMenu:GetPlayerCoords", serverId, CallbackFunction);
        ...
        {
            xMenuShared.PermissionsManager.SetPermissions(permissionsList);
        ...
        {
            switch (GetSettingsInt(Setting.xmenu_pvp_mode))
            {
                case 1:
            ...
            bool canUseMenu()
            {
                if (GetSettingsBool(Setting.xmenu_menu_staff_only) == false) return true;
            ...
            if (GetSettingsInt(Setting.xmenu_menu_toggle_key) != -1)
            {
                MenuToggleKey = (Control)GetSettingsInt(Setting.xmenu_menu_toggle_key);
                //MenuToggleKey = GetSettingsInt(Setting.xmenu_menu_toggle_key);
            }
            if (GetSettingsInt(Setting.xmenu_noclip_toggle_key) != -1)
            {
                NoClipKey = GetSettingsInt(Setting.xmenu_noclip_toggle_key);
            }
            ...
            if (!GetSettingsBool(Setting.xmenu_disable_player_stats_setup))
            {
                // Manage Stamina
            ...
            TriggerEvent("xMenu:SetupTickFunctions");
        }
        ...
        private static void CreateSubmenus()
        {
            ...
            if (IsAllowed(Permission.OPUnban) || IsAllowed(Permission.OPViewBannedPlayers))
            {
                ...
                Menu.OnItemSelect += (sender, item, index) =>
                {
                    if (item == button)
                    {
                        TriggerServerEvent("xMenu:RequestBanList", Game.Player.Handle);
                        menu.RefreshIndex();
                    }
                };
            }
            ...
            if (IsAllowed(Permission.SXMenu))
            {
            ...
            if (IsAllowed(Permission.PXMenu))
            {
            ...
            if (IsAllowed(Permission.PAMenu))
            {
                ...
                MenuItem button2 = new MenuItem("MP Ped Customization", "Create, edit, save and load multiplayer peds. ~r~Note, you can only save peds created in this submenu. xMenu can NOT detect peds created outside of this submenu. Simply due to GTA limitations.")
                {
                    Label = "→→→"
                };
            ...
            if (IsAllowed(Permission.TOMenu) && GetSettingsBool(Setting.xmenu_enable_time_sync))
            {
            ...
            // check for 'not true' to make sure that it _ONLY_ gets disabled if the owner _REALLY_ wants it disabled, not if they accidentally spelled "false" wrong or whatever.
            if (IsAllowed(Permission.WOMenu) && GetSettingsBool(Setting.xmenu_enable_weather_sync))
            {
            ...
            // Add misc settings menu.
            {
                ...
                MenuItem button = new MenuItem("Misc Settings", "Miscellaneous xMenu options/settings can be configured here. You can also save your settings in this menu.")
                {
                    Label = "→→→"
                };
                AddMenu(Menu, menu, button);
            ...
            MenuItem btn = new MenuItem("About xMenu", "Information about xMenu.")
            {
                Label = "→→→"
            };
            ...
            {
                Notify.Alert("xMenu is set up to ignore permissions, default permissions will be used.");
            }
