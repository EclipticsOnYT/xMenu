...
namespace xMenuClient
{
    public class WeaponLoadouts
    {
        ...
        {
            int handle = StartFindKvp("xmenu_string_saved_weapon_loadout_");
        ...
        // xmenu_temp_weapons_loadout_before_respawn
        // xmenu_string_saved_weapon_loadout_
        ...
        public void CreateMenu()
        {
            ...
            void RefreshSavedWeaponsMenu()
            {
                ...
                {
                    MenuItem btn = new MenuItem(sw.Key.Replace("xmenu_string_saved_weapon_loadout_", ""), "Click to manage this loadout.") { Label = "→→→" };
            ...
            menu.OnItemSelect += async (sender, item, index) =>
            {
                if (item == saveLoadout)
                {
                    ...
                    {
                        if (SavedWeapons.ContainsKey("xmenu_string_saved_weapon_loadout_" + name))
                        {
                            Notify.Error(CommonErrors.SaveNameAlreadyExists);
                        }
                        else
                        {
                            if (SaveWeaponLoadout("xmenu_string_saved_weapon_loadout_" + name))
                            {
            ...
            ManageLoadoutMenu.OnItemSelect += async (sender, item, index) =>
            {
                if (SavedWeapons.ContainsKey(SelectedSavedLoadoutName))
                {
                    ...
                    {
                        string newName = await GetUserInput("Enter a save name", SelectedSavedLoadoutName.Replace("xmenu_string_saved_weapon_loadout_", ""), 30);
                        ...
                        {
                            if (SavedWeapons.ContainsKey("xmenu_string_saved_weapon_loadout_" + newName))
                            {
                                Notify.Error(CommonErrors.SaveNameAlreadyExists);
                            }
                            else
                            {
                                SetResourceKvp("xmenu_string_saved_weapon_loadout_" + newName, JsonConvert.SerializeObject(weapons));
                    ...
                    {
                        SetResourceKvp("xmenu_string_default_loadout", SelectedSavedLoadoutName);
            ...
            {
                if (SavedWeapons.ContainsKey("xmenu_string_saved_weapon_loadout_" + item.Text))
                {
                    SelectedSavedLoadoutName = "xmenu_string_saved_weapon_loadout_" + item.Text;
                }
            ...
            ManageLoadoutMenu.OnMenuOpen += (sender) =>
            {
                ...
                string kvp = GetResourceKvpString("xmenu_string_default_loadout");
