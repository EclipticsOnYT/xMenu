    ...
    public class PlayerAppearance
    {
        ...
        private void CreateMenu()
        {
            ...
            if (IsAllowed(Permission.PASpawnNew))
            {
                ...
                if (xMenuShared.ConfigManager.GetSettingsBool(xMenuShared.ConfigManager.Setting.xmenu_enable_animals_spawn_menu))
                {
                    ...
                    if (propID == 0)
                    {
                        ...
                        if (N_0xd40aac51e8e4c663((uint)compHash) > 0) // helmet has visor. 
                        {
                            if (!IsHelpMessageBeingDisplayed())
                            {
                                ...
                                AddTextComponentSubstringPlayerName("when on foot or on a motorcycle and when xMenu is closed.");
