...
namespace xMenuClient
{
    public class MiscSettings
    {
        ...
        public int KbTpToWaypointKey { get; } = xMenuShared.ConfigManager.GetSettingsInt(xMenuShared.ConfigManager.Setting.xmenu_teleport_to_wp_keybind_key) != -1
            ? xMenuShared.ConfigManager.GetSettingsInt(xMenuShared.ConfigManager.Setting.xmenu_teleport_to_wp_keybind_key)
        ...
        internal static List<xMenuShared.ConfigManager.TeleportLocation> TpLocations = new List<xMenuShared.ConfigManager.TeleportLocation>();
        ...
        private void CreateMenu()
        {
            ...
            MenuCheckboxItem rightAlignMenu = new MenuCheckboxItem("Right Align Menu", "If you want xMenu to appear on the left side of your screen, disable this option. This option will be saved immediately. You don't need to click save preferences.", MiscRightAlignMenu);
            ...
            MenuItem saveSettings = new MenuItem("Save Personal Settings", "Save your current settings. All saving is done on the client side, if you re-install windows you will lose your settings. Settings are shared across all servers using xMenu.")
            {
            ...
            if (IsAllowed(Permission.MSTeleportToWp) || IsAllowed(Permission.MSTeleportLocations) || IsAllowed(Permission.MSTeleportToCoord))
            {
                ...
                if (IsAllowed(Permission.MSTeleportLocations))
                {
                    ...
                    {
                        if (item.ItemData is xMenuShared.ConfigManager.TeleportLocation tl)
                        {
            ...
            // model outlines
            if (!xMenuShared.ConfigManager.GetSettingsBool(xMenuShared.ConfigManager.Setting.xmenu_disable_entity_outlines_tool))
            {
            ...
            developerToolsMenu.OnItemSelect += (sender, item, index) =>
            {
                if (item == clearArea)
                {
                    ...
                    BaseScript.TriggerServerEvent("xMenu:ClearArea", pos.X, pos.Y, pos.Z);
                }
            };
        ...
        private void ToggleBlips(bool enable)
        {
            if (enable)
            {
                try
                {
                    foreach (var bl in xMenuShared.ConfigManager.GetLocationBlipsData())
                    {
                ...
                {
                    Debug.Write($"\n\n[xMenu] An error occurred while loading the locations.json file. Please contact the server owner to resolve this.\nWhen contacting the owner, provide the following error details:\n{ex.Message}.\n\n\n");
                }
            }
            else
            {
