...
namespace xMenuClient
{
    public static class CommonFunctions
    {
        ...
        public static async void QuitGame()
        {
            ...
            Debug.WriteLine("Game will be terminated in 5 seconds, because the player used the Quit Game option in xMenu.");
        ...
        public static async void KickPlayer(IPlayer player, bool askUserForReason, string providedReason = "You have been kicked.")
        {
            if (player != null)
            {
                ...
                TriggerServerEvent("xMenu:KickPlayer", player.ServerId, defaultReason);
        ...
        public static async void BanPlayer(IPlayer player, bool forever)
        {
            ...
            if (!string.IsNullOrEmpty(banReason) && banReason.Length > 1)
            {
                if (forever)
                {
                    TriggerServerEvent("xMenu:PermBanPlayer", player.ServerId, banReason);
                }
                else
                {
                    ...
                    if (!string.IsNullOrEmpty(banDurationHours))
                    {
                        ...
                        {
                            if (banHours > 0.0)
                                TriggerServerEvent("xMenu:TempBanPlayer", player.ServerId, banHours, banReason);
                            else
                        ...
                        else
                        {
                            if (int.TryParse(banDurationHours, out int banHoursInt))
                            {
                                if ((double)banHoursInt > 0.0)
                                    TriggerServerEvent("xMenu:TempBanPlayer", player.ServerId, (double)banHoursInt, banReason);
                                else
                                    Notify.Error("You need to enter a ban duration, enter a value ~h~between~h~ 1 and 720!");
                            }
                            else
                            {
                                ...
                                TriggerEvent("chatMessage", $"[xMenu] The input is invalid or you cancelled the action, please try again.");
                            }
                        }
                    }
                    else
        ...
        /// </summary>
        /// <param name="player"></param>
        public static void KillPlayer(IPlayer player) => TriggerServerEvent("xMenu:KillPlayer", player.ServerId);
        ...
        /// </summary>
        /// <param name="player"></param>
        public static void SummonPlayer(IPlayer player) => TriggerServerEvent("xMenu:SummonPlayer", player.ServerId);
        #endregion
        ...
        public static async Task<int> SpawnVehicle(uint vehicleHash, bool spawnInside, bool replacePrevious, bool skipLoad, VehicleInfo vehicleInfo, string saveName = null, float x = 0f, float y = 0f, float z = 0f, float heading = -1f)
        {
            ...
            if (_previousVehicle != null)
            {
                    ...
                    (_previousVehicle.Occupants.Count() == 0 || _previousVehicle.Driver.Handle == Game.PlayerPed.Handle))
                {
                    ...
                    {
                        if (!xMenuShared.ConfigManager.GetSettingsBool(xMenuShared.ConfigManager.Setting.xmenu_keep_spawned_vehicles_persistent))
                        {
        ...
        /// <returns></returns>
        //public static bool IsAllowed(xMenuShared.PermissionsManager.Permission permission) => xMenuShared.PermissionsManager.IsAllowed(permission);//PermissionsManager.IsAllowed(permission);
        #endregion
        ...
        public static void UpdateServerWeather(string newWeather, bool blackout, bool dynamicChanges, bool isSnowEnabled) => TriggerServerEvent("xMenu:UpdateServerWeather", newWeather, blackout, dynamicChanges, isSnowEnabled);
        ...
        public static void ModifyClouds(bool removeClouds) => TriggerServerEvent("xMenu:UpdateServerWeatherCloudsType", removeClouds);
        #endregion
        ...
        public static void UpdateServerTime(int hours, int minutes, bool freezeTime)
        {
            ...
            TriggerServerEvent("xMenu:UpdateServerTime", realHours, realMinutes, freezeTime);
        }
        #endregion
        ...
        public static async Task SetPlayerSkin(uint modelHash, PedInfo pedCustomizationOptions, bool keepWeapons = true)
        {
            if (IsModelInCdimage(modelHash))
            {
                if (keepWeapons)
                {
                    SaveWeaponLoadout("xmenu_temp_weapons_loadout_before_respawn");
                ...
                if (keepWeapons)
                {
                    await SpawnWeaponLoadoutAsync("xmenu_temp_weapons_loadout_before_respawn", false, true, false);
                }
        ...
        public static async Task<bool> SavePed(string forceName = null, bool overrideExistingPed = false)
        {
            ...
            if (!string.IsNullOrEmpty(name))
            {
                ...
                //if (name != "xMenu_tmp_saved_ped") // only send a notification if the save wasn't triggered because the player died.
                //{
                //    // If the save was successfull.
                //    if (saveSuccessful)
                //    {
        ...
        {
            if (savedName != "xMenu_tmp_saved_ped")
            {
            ...
            else
            {
                ...
                Log(JsonConvert.SerializeObject(pi));
                await SetPlayerSkin(pi.model, pi, restoreWeapons);
                DeleteResourceKvp("xMenu_tmp_saved_ped");
            }
        ...
        {
            if (!string.IsNullOrEmpty(GetResourceKvpString("xMenu_tmp_saved_ped")))
            {
                return true;
            }
            return false;
        ...
        #region Log Function
        /// <summary>
        /// Print data to the console and save it to the CitizenFX.log file. Only when xMenu debugging mode is enabled.
        /// </summary>
        ...
        {
            if (saveName == "xmenu_temp_weapons_loadout_before_respawn")
            {
                return JsonConvert.DeserializeObject<List<ValidWeapon>>(GetResourceKvpString("xmenu_temp_weapons_loadout_before_respawn") ?? "{}");
            }
            else
            {
                string kvp = GetResourceKvpString(saveName.StartsWith("xmenu_string_saved_weapon_loadout_") ? saveName : "xmenu_string_saved_weapon_loadout_" + saveName);
        ...
        public static async Task SpawnWeaponLoadoutAsync(string saveName, bool appendWeapons, bool ignoreSettingsAndPerms, bool dontNotify)
        {
            ...
            if (!ignoreSettingsAndPerms && saveName == "xmenu_temp_weapons_loadout_before_respawn")
            {
                string name = GetResourceKvpString("xmenu_string_default_loadout") ?? saveName;

                string kvp = GetResourceKvpString(name) ?? GetResourceKvpString("xmenu_temp_weapons_loadout_before_respawn");
                ...
                {
                    kvp = GetResourceKvpString("xmenu_temp_weapons_loadout_before_respawn");
            ...
            if (loadout.Count > 0)
            {
                ...
                if (!(saveName == "xmenu_temp_weapons_loadout_before_respawn" || dontNotify))
        ...
        */
        ...
        public static async void PrivateMessage(string source, string message, bool sent)
        {
            ...
            if (MainMenu.MiscSettingsMenu == null || MainMenu.MiscSettingsMenu.MiscDisablePrivateMessages)
            {
                ...
                {
                    TriggerServerEvent("xMenu:PmsDisabled", source);
                }
                return;
            }
        ...
        public static async void SavePlayerLocationToLocationsFile()
        {
            ...
            if (xMenuShared.ConfigManager.GetTeleportLocationsData().Any(loc => loc.name == locationName))
            {
                Notify.Error("This location name is already used, please use a different name.");
                return;
            }
            TriggerServerEvent("xMenu:SaveTeleportLocation", JsonConvert.SerializeObject(new xMenuShared.ConfigManager.TeleportLocation(locationName, pos, heading)));
